...
namespace LocalizeMaster
{
    public partial class MainWindow : Window
    {
        ...
        {
            var fluentTheme = new FluentTheme(new System.Uri("avares://Avalonia.Themes.Fluent/FluentTheme.xaml"));
            fluentTheme.Mode = System.Enum.TryParse(themeName, out FluentThemeMode res) ? res : FluentThemeMode.Dark;
            Application.Current.Styles[0] = fluentTheme;
        }
        ...
        private async void Term_PropertyChanged(object? sender, PropertyChangedEventArgs e)
        {
            switch(e.PropertyName)
            {
                case nameof(Term.Value):
                    if (sender is Term term && !string.IsNullOrWhiteSpace(term.Value) && this.Values.Any(x => x != term && x.Value == term.Value))
                    {
        ...
        {
            await new SettingsWindow() { DataContext = Settings}.ShowDialog(this);
        }
            ...
            if (folder != null)
                foreach (var lng in this.Languages)
                    File.WriteAllText(Path.Combine(folder, $"{lng.CultureName}.json"),
                        JsonConvert.SerializeObject(lng.Items
                        .GroupBy(x => x.Key.Value)
                        .Where(x => (Settings.SerializeEmptyTerms ? true : !string.IsNullOrWhiteSpace(x.Key))
                        && (Settings.SerializeEmptyValues ? true : !string.IsNullOrWhiteSpace(x.First().Value)))
        ...
        protected override async void OnClosing(CancelEventArgs e)
        {
            ...
            {
                await MessageBoxManager.GetMessageBoxStandardWindow("Error", "Can't save settings").ShowDialog(this);
            }
        }
